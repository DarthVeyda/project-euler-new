...
namespace ProjectEuler
{
    internal class Problem27 : BaseProblem
    {
        ...
        public Problem27(int problemNumber, long inputParam, string outputTemplate)
        ...
        {
            long aMax = -InputParam;
            long bMax = -InputParam;
            int countMax = 0;
            for (long a = -InputParam; a <= InputParam; a++)
                for (long b = -InputParam; b < InputParam; b++)
                {
                    int n = 0;
                    int count = 0;
                    while (CommonFunctions.IsPrime(n*n + a*n + b))
                    {
                        n++;
                        count++;
                    }
